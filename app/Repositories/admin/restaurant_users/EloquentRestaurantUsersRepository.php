...
//        }
        ...
        if (isset($data['updated_at']) && !empty($data['updated_at'])) {
         ...
         if (isset($data['mobile']) && !empty($data['mobile'])) {
             ...
             $ds= DB::table('users')->where('users.mobile', 'like', $data['mobile'])
             ->lists('id','id');
            $oResults = $oResults->whereIn('users_id',$ds);
...
//    }
   ...
   public function exportUserResturant($request){
        ...
        $excelData=$this->UserResturantToArray($oResults);

    ob_end_clean();

    ob_start();
        \Excel::create('RestaurantUser', function($excel) use($excelData) {
 ...
 public function UserResturantToArray($oResults){
        $aResults=[['id','users_id','restaurant_id','status','type']];
        if(count($oResults)){
