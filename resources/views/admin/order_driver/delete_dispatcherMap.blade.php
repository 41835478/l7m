...
<div id="right-panel">
    ...
    <select name="selected_restaurant_id" onchange="$('#selectDateForm').submit();">
            ...
            <div id="DriverList">
                ...
                @foreach($oRestaurantUsers as &$oRestaurantUser)
                    ...
                    <label class="oneDriver">
                     ...
                     <div class="popupHtml" id="driver_{{$oRestaurantUser->users_id}}">


                         {{$oRestaurantUser->users->first_name  }} -<span >  {{ $oRestaurantUser->restaurant->name_en  }} <img src="{{$oRestaurantUser->restaurant->img}}" style="width:20px;height: 20px;" ></span> <br>
                        status :   <a href="/admin/restaurant_users/{{$oRestaurantUser->id}}/edit">{{ config('array.restaurant_users_status')[$oRestaurantUser->status] }}</a>
 mobile : {{$oRestaurantUser->users->mobile  }} - phone : {{$oRestaurantUser->users->phone  }}
...
@foreach($oRestaurantUser->order as $key=>$order)
    ...
    @endforeach
                         <button type="button" onclick="$('#driver_radio_{{$oRestaurantUser->users_id }}').prop('checked',true);$('#submit').click();">Start From here</button>
            ...
            <div multiple id="waypoints">
                ...
                @foreach($oOrderResults as $oOrderResult)
                    ...
                    <label class="oneOrder" >
                        ...
                        <input type="checkbox" id="order_checkbox_{{$oOrderResult->id}}" name="order_id[]" value="{{$oOrderResult->id}}" data-location="{{isset($oOrderResult->address->lat)? $oOrderResult->address->lat.','.$oOrderResult->address->long:''}}"
                                @if(isset($oOrderResult->address->lat) &&strlen($oOrderResult->address->lat) >5 && strlen($oOrderResult->address->long) >5) checked @endif
                                >

                        <a href="/admin/order/{{$oOrderResult->id}}/edit" >#{{$oOrderResult->id}} {{ isset($oOrderResult->users->first_name)? $oOrderResult->users->first_name:''}}</a>
                        <br>{{config('array.order_status')[$oOrderResult->status] }} -<a href="/admin/address/{{$oOrderResult->address_id}}"> {{isset($oOrderResult->address->name)? $oOrderResult->address->name:'' }}</a> - <a href="/admin/restaurant/{{$oOrderResult->restaurant_id}}" ></a>{{isset($oOrderResult->restaurant->name_en)?$oOrderResult->restaurant->name_en:''}}</a>

                    <span style="float:right;" class="pull-right">{{$oOrderResult->deliver_time}}
                        <input name="expected_time[{{$oOrderResult->id}}]" value=""  type="xxx_hidden" id="expected_time_{{$oOrderResult->id}}">
                    </span>
                    </label>
...
<img src="{{isset($oOrderResult->restaurant->img)? $oOrderResult->restaurant->img:''}}" width="100" height="100" >
                        <button onclick="$('#order_checkbox_{{$oOrderResult->id}}').prop('checked',false);$('#submit').click();">remove</button>
                        <button onclick="$('#order_checkbox_{{$oOrderResult->id}}').prop('checked',true);$('#submit').click();">Add</button>
                    </div>
