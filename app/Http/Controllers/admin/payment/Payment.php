...
class Payment extends Controller
{
     ...
     * @return void
     */
    public function index(Request $request,rRestaurnat $rRestaurnat,rArea $rArea,rUsers $rUsers, rOrder $rOrder)
    {
        ...
        return view('admin.payment.index', compact('oResults', 'request', 'statistic','restaurantList','areaList','usersList', 'orderList'));
    }
  ...
  if(isset(current_user()->getUser()->id ) && current_user()->getUser()->type==config('array.users_type_guest_index'))
  {
    ...
    $email=current_user()->getUser()->email;
        ...
        \Mail::send('website.payment.knet.success_email', ['request'=>$request,'oPayment'=>$oPayment], function($message,$email)
        {
