        ...
        if (count($oOrder) && count($oOrder->cart)) {
            ...
            foreach ($oOrder->cart as $cart) {
                ...
                if ($oRestaurant->discount_type == config('array.restaurant_discount_type_percentage_index')) {
                   $unit_price=floatval($cart->unit_price);
                  //if decount return error data please check the dicount  $discount
                    $totalDiscount += ($unit_price * $cart->quantity * (float) $discount / 100);
                   
                } else {

                   // $totalDiscount += ($cart->quantity * $discount);

                      $totalDiscount +=($cart->quantity * (float) $discount / 100);
                }
