            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="white-box">
                        ...
                        <tr>
                            <td colspan="3">Total Orders Price  </td><td colspan="6">{{ abs($totalOrderPrice)}} {{trans('general.kd')}}</td>
                        </tr>
                        <tr>
                            <td colspan="3">  Payment Type Commission  </td><td colspan="6">{{ $totalOrderCommission }} {{trans('general.kd')}}</td>
                        </tr>


                        <tr>
                            <td colspan="3">Total Sahalat Charge </td><td colspan="6">{{ abs($sahalatCharge)}}</td>
                        </tr>
                        @if(isset($oDeliverPackage->id))
                        <tr>

                            <td colspan="3">Package Charge   </td><td colspan="6">
                                Deliver Number Package (  {{ isset($oDeliverPackage->deliver_number)? $oDeliverPackage->deliver_number:0 }} ) :
                                {{--*/
                                $packageDiscount=isset($oDeliverPackage->price)? $oDeliverPackage->price:0;
                                /*--}}
                                {{$totalOrder}}&times; {{ $packageDiscount }} = {{ $totalOrder * $packageDiscount  }}
                            </td>
                        </tr>
                        @endif
                        @if(count($oRestaurantOffer))
                        <tr>
                            <td colspan="3">Offer discount </td><td colspan="6">

                                @foreach($oRestaurantOffer as $restaurantOffer)
                                    @if(isset($restaurantOffer->offer->id))
                                       The commission for ( {{ $restaurantOffer->offer->deliver_number}} ) deliver  :
                                        {{ $restaurantOffer->offer->price  == 0 ? "Free":$restaurantOffer->offer->price }}
                                        <br>
                            ...
                            <tr>
                               ...
                               Payment Type
                                ...
                                <th scope="col" data-tablesaw-sortable-col data-tablesaw-priority="10">
                                    {!! th_sort(trans('order.sahalat_charge'), 'sahalat_charge', $oResults) !!}
                                </th>
...
{{--                                        <td>{{ $oResult->total }}</td>--}}
                                                ...
                                                @if (count($oResult->cart))
                                                    ...
                                                    @foreach($oResult->cart as $cart)
                                                        ...
                                                        <div class='{{ $class1 }}' style="display: table-row;clear:both;" >
                                                            ...
                                                            <div style="display: table-cell;">{{ $cart->dish->name_en  }}</div>
                                                            <div style="display: table-cell;">{{ $cart->unit_price  }}</div>
                                                            <div style="display: table-cell;">{{ $cart->quantity  }}</div>

                                                            <div  style="display: table-cell;">{{ floatval( $cart->quantity) *floatval($cart->unit_price) }}</div>
        ...
        <div class="right-side-panel">
            <div class="scrollable-right container">
                ...
                <div class="form-group">
                    <div class="col-md-12">
                        {!! Form::select('dish_id',[''=>'Select Dish']+$dishList, null, ['placeholder'=>'Dish','class'=>'form-control input-sm ']) !!}
