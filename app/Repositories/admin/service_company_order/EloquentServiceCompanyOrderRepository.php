...
class EloquentServiceCompanyOrderRepository implements ServiceCompanyOrderContract
{
    ...
    public function getByFilter($data)
    {
        ...
        $oResults =  ServiceCompanyOrder::with('service_boy','driver','service');
        ...
        if (isset($data['service_boy_id']) && !empty($data['service_boy_id'])) {
            $oResults = $oResults->where('service_boy_id', '=', $data['service_boy_id']);
        }

        if (isset($data['notAssigned']) && !empty($data['notAssigned'])) {
            $oResults = $oResults->where(function($query){
                $query->where('service_boy_id', '=', 0);
                $query->orWhere('service_boy_id', '=', '');
            });
        }
