...
use App\module\notification\admin\controller\android\PushNotification as rNotification;

use App\module\notification\admin\controller\ios\PushNotification as iosNtification;
...
class OrderDriver extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->ajax()){
            return  json_encode(['status'=>'success','rows'=>$oResults->all()]);
...
//        return view('admin.order_driver.create',compact('request','usersList','orderList'));
     ...
     * @return redirect
     */
    public function updatePublic(editRequest $request)
    {

        $result = $this->rOrderDriver->update($request->id, $request);
 ...
 /**
     ...
     * @return redirect
     */
    public function updatePublicGroup(Request $request)
    {

      foreach ($request->data as  $id) {
            $result = $this->rOrderDriver->update($id,['status'=> $request->status]);
      }
...
/**
    ...
    public function UpdatReport(Request $request)
    {
      ...
      $result =  mOrderDriver::find($request->id)->update([
                    'note' => $request->note,
                     ]);
 ...
 public function SendNotification(Request $request , mUsers $mUsers,rNotification  $rNotification, iosNtification $iosNtification)

 {
    ...
    if(isset($request->android_device_id))
        
    {  
            $id= $request->user_id;
            $results = $mUsers::find($id);

            $result = $rNotification->send($results->android_device_id,$request->title,$request->body);
        ...
        if(isset($request->ios_device_id))

        {
             $id= $request->user_id;
            $results = $mUsers::find($id);
    
        $result = $iosNtification->send($results->ios_device_id,$request->message,$request->body);
            return  json_encode(['status'=>'success']);
    ...
    public function getDispatcherMap(
        ...
        $oRestaurantResults = $rRestaurant->getByFilter(['getAllRecords'=>1]);
