...
class EloquentOrderDriverRepository implements OrderDriverContract
{
    ...
    public function getByFilter($data)
    {
        ...
        $oResults =  OrderDriver::with(['users','order'=>function($query){$query->with('restaurant','address');}]);
...
$chartData=[];
 ...
 $aResults=[['ID','Driver Name','Order id','Status','deliver start hour','deliver end hour','notification','updated at']];
   ...
   (isset($oResult->deliver_start_hour))?$oResult->deliver_start_hour:'' ,

   (isset( $oResult->deliver_end_hour))? $oResult->deliver_end_hour:'' ,
